document.addEventListener('DOMContentLoaded', function () {
    ...
    if (preferredLang) {
        const currentLang = window.location.pathname.includes('en.html') ? 'en' : 'ru';
        if (preferredLang !== currentLang) {
            window.location.href = preferredLang === 'en' ? 'en.html' : 'index.html';
        }
    }
});
...
function initLicenseModal() {
    ...
    const modalImg = document.getElementById("licenseImg");
    const span = document.querySelector(".close-modal");

    if (!img || !modal || !modalImg || !span) return;
